...
<div class="col-md-12 col-sm-12">
    ...
    <div class="row">
        @if ($tranche)
            @foreach($tranche as $proget)        
            <?php $date = date("y", strtotime($proget->datainizio));?>
     <div class="col-md-4 col-sm-6 col-lg-3">
            <div class="innr-img">
                <a href="{{url("/pagamenti/tranche/pdf/$proget->tranche_id")}}">
                    <img src="{{asset('storage/app/images/pdf.png')}}" alt="" class="img-fluid"></a><p> 
                        <a href="{{url("/pagamenti/tranche/pdf/$proget->tranche_id")}}">#{{$proget->tranche_id}}|{{$date}}--{{$proget->nomeprogetto}}</a>
            </div>
        </div>
        @endforeach
        
        @else
        <b>{{"Nessun elemento trovato"}}</b>
        @endif

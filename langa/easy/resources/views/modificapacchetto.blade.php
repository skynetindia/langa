...
<?php echo Form::open(array('url' => '/admin/tassonomie/update/pacchetto' . "/$pacchetto->id", 'files' => true)) ?>
	...
	<div class="col-md-4">
		...
		<input value="{{ $pacchetto->code }}" class="form-control" type="text" name="code" id="code" placeholder="Codice"><br>
	...
	<div class="col-md-4">
		...
		<input value="{{ $pacchetto->label }}" class="form-control" type="text" name="label" id="label" placeholder="Nome"><br>
                        ...
                        <td class="ciao">
                            @foreach($optionalselezionati as $optsel)
                                @if($optsel->optional_id == $optional[$i]->id)
                                    <input checked type="checkbox" name="optional[]" id="optional[]" value="<?php echo $optional[$i]->id; ?>"><?php echo " " . $optional[$i]->label; ?>
                                    <?php $check = false; ?>
                                @endif
                            @endforeach
                            @if($check==true)
